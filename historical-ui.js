...
function historicalSeasonSummaryHtml(name, season = "") {
  ...
  const body = [...groups].sort(([left], [right]) => String(right).localeCompare(String(left))).map(([seasonId, weeks]) => {
    ...
    const weekly = season ? weeks.sort((left, right) => Number(left.weekOrder || left.week) - Number(right.weekOrder || right.week)).map(row => `<div class="tooltip-week-row" data-weekly-drilldown data-player-id="${websiteEscapeHtml(row.playerId)}" data-season="${websiteEscapeHtml(row.season)}" data-week="${websiteEscapeHtml(row.week)}"><span>${profileWeekLabel(row.week)}</span><span>#${row.rank || "-"}</span><span>${row.displayWins}-${row.displayLosses}${Number(row.displayPushes || 0) ? `-${row.displayPushes}` : ""}</span></div>`).join("") : "";
