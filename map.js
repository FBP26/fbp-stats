...
function fbpMapTooltip(row, metric, totalViews) {
  ...
  if (row.kind === "visitor") {
    ...
    const players = (row.players || []).join(", ");
    return `<div class="fbp-map-tooltip-title">${websiteEscapeHtml(location)}</div>${players ? `<div class="fbp-map-tooltip-subtitle">${websiteEscapeHtml(players)}</div>` : ""}<div class="fbp-map-tooltip-grid">${cell("Page views", views.toLocaleString())}${cell("Traffic share", share)}${cell("Unique browsers", Number(row.uniqueBrowsers || 0).toLocaleString())}${cell("Sessions", sessions.toLocaleString())}${cell("Views / session", viewsPerSession)}${cell("Pick starts", starts.toLocaleString())}${cell("Tracked submits", submissions.toLocaleString())}${cell("Start → submit", conversion)}</div><div class="fbp-map-tooltip-note">Analytics events only, not total pool entries · Approximate network location · Updated ${websiteEscapeHtml(updated)}</div>`;
  }
  if (row.kind === "venue") {
...
function fbpMapRenderDetail(row, metric) {
  ...
  if (row.kind === "visitor") {
    ...
    host.innerHTML = `<div class="fbp-map-rail-head"><div><span class="fbp-map-rail-kicker">Approximate visitor geography</span><h3>${websiteEscapeHtml(location)}</h3></div></div>${players.length ? `<p class="fbp-map-caption">Known players seen here: ${websiteEscapeHtml(players.join(", "))}</p>` : ""}<div class="fbp-map-stat-grid"><div><span>Page views</span><strong>${Number(row.views || 0).toLocaleString()}</strong></div><div><span>Unique browsers</span><strong>${Number(row.uniqueBrowsers || 0).toLocaleString()}</strong></div><div><span>Sessions</span><strong>${Number(row.sessions || 0).toLocaleString()}</strong></div><div><span>Pick starts</span><strong>${Number(row.pickStarts || 0).toLocaleString()}</strong></div><div><span>Tracked submits</span><strong>${Number(row.submissions || 0).toLocaleString()}</strong></div></div><p class="fbp-map-caption">These are analytics events from browsers that allowed tracking, not total pool entries. Location is approximate from Cloudflare. A player's name is shown only when that browser has submitted picks; no IP addresses or raw browser/session identifiers are included in this Map data.</p>`;
    return;
  }
  if (row.kind === "venue") {
    const title = row.names.at(-1) || row.venueId;
...
function fbpMapSessionRows() {
  return (fbpMapVisitorData.sessions || []).map(session => {
    ...
    const sessionLabel = session.playerName ? websiteEscapeHtml(session.playerName) : `Session #${Number(session.sessionNumber || 0).toLocaleString()}`;
    return `<tr><th scope="row"><span class="fbp-map-session-number">${sessionLabel}</span><small class="fbp-map-session-location">${websiteEscapeHtml(location)}</small></th><td>${websiteEscapeHtml(startText)}</td><td>${websiteEscapeHtml(activeTime)}</td><td><div class="fbp-map-session-bar" aria-label="Session page timeline">${pages}</div></td></tr>`;
  }).join("");
}
